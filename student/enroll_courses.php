...
if ($conn) {
    ...
    ";
    if (!mysqli_query($conn, $create_enrollments_query)) {
        // Log error if table creation fails
        error_log("Enrollments table creation failed: " . mysqli_error($conn));
    }

    // B. Ensure the 'courses' table exists (if it doesn't, this query will create it with 'status')
    ...
    ";
    if (!mysqli_query($conn, $create_courses_query)) {
        error_log("Courses table creation failed: " . mysqli_error($conn));
    }

    // C. Add 'status' column to 'courses' table if it is missing 
    // This specifically resolves the "Unknown column 'status'" fatal error if the table existed without it.
    ...
    if ($result && mysqli_num_rows($result) === 0) {
        ...
        if (!mysqli_query($conn, $alter_table_query)) {
            error_log("Adding 'status' column failed: " . mysqli_error($conn));
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['course_id'])) {
    ...
    // Input validation
    if ($course_id <= 0) {
        $error_message = "Invalid course ID submitted.";
    } else {
        // Check if student is already enrolled (prevents duplicates and query failure)
        ...
        if ($count > 0) {
            ...
            if ($stmt_enroll->execute()) {
                ...
                // If a unique constraint violation occurs (should be caught by the check above, but as a safeguard)
                $error_message = "Enrollment failed. Could not add you to the course. Error: " . $stmt_enroll->error;
            }
            $stmt_enroll->close();
        }
    }
...
$available_courses = [];
if ($conn) {
    // This query is now safe because the column check/alter logic in step 1 ran successfully.
    ...
    if ($result_courses) {
        ...
        // Fallback or logging if the primary query fails
        error_log("Failed to fetch active courses: " . mysqli_error($conn));
        // Fallback to fetching all courses if 'status' filter fails (unlikely after alter)
        ...
        if ($result_courses) {
            ...
            $error_message .= " Severe Error: Could not retrieve any courses.";
        }
    }
}
